...
class GeneralStateSpaceModel(ABC):
    def __init__(self, state_dim: int, obs_dim: int, dt: float = 0.01) -> None:
    ...
    def plot_simulation(
        ...
        obs_dims: Optional[List[int]] = None,
        traj_state_dims: Optional[List[int]] = None,
        traj_obs_dims: Optional[List[int]] = None,
        ...
        if len(traj_state_dims) >= 2:
            ...
            ax.plot(
                states[:, traj_state_dims[0]],
                states[:, traj_state_dims[1]],
                label="State Trajectory",
                zorder=1,
            )
            ax.scatter(
                states[0, traj_state_dims[0]],
                states[0, traj_state_dims[1]],
                color="green",
                marker="o",
                s=100,
                label="Start",
            ...
            ax.scatter(
                states[-1, traj_state_dims[0]],
                states[-1, traj_state_dims[1]],
                color="red",
                marker="x",
                s=100,
                label="End",
        ...
        if len(traj_obs_dims) >= 2:
            fig, ax = plt.subplots(figsize=(6, 6))
            ax.plot(
                observations[:, traj_obs_dims[0]],
                observations[:, traj_obs_dims[1]],
                label="Observation Trajectory",
                zorder=1,
            )
            ax.scatter(
                observations[0, traj_obs_dims[0]],
                observations[0, traj_obs_dims[1]],
                color="green",
                marker="o",
                s=100,
                label="Start",
            ...
            ax.scatter(
                observations[-1, traj_obs_dims[0]],
                observations[-1, traj_obs_dims[1]],
                color="red",
                marker="x",
                s=100,
                label="End",
    ...
    @staticmethod
    def load_model(filename: str) -> "GeneralStateSpaceModel":
